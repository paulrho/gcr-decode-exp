...
{
//fprintf(stderr,"In rfi_writetrack\n");
  if (rfifile==NULL) return;
...
//printf("In rfi_writetrack 2\n");
    ...
    if (rledata!=NULL)
    {
      ...
      if (11111) {
	      // special little thing to write just this track to track.rle - for offloading processing to r15
	      FILE *frle=fopen("track.rle","wb");
	      if (frle!=NULL) {
                fwrite(rledata, 1, rledatalength, frle);
		fclose(frle);
  ...
  {
//printf("In rfi_writetrack 3\n");
    // Don't write any track data for unknown encodings
    fprintf(rfifile, "enc:\"unknown\",len:0}");
  }
//printf("In rfi_writetrack 4\n");
}
