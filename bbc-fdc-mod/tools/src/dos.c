...
void dos_showinfo_display(const unsigned int disktracks, const unsigned int debug)
{
  Disk_Sector *sector1;
  struct dos_biosparams *biosparams;
  ...
  sprintf(str,"%.4x-%.4x", exbiosparams->volumeserial>>16, exbiosparams->volumeserial&0xffff);
  display_info_add("Volume ID serial",str);
  sprintf(str,"%d", biosparams->smallsectors);
  display_info_add("Sectors on vol",str);
  ...
  printf("'\n");
      display_info_add("Volume label",str);
  ...
  printf("'\n");
      display_info_add("System ID",str);
  ...
  switch (fatformat)
  {
    case DOS_FAT12:
      printf("FAT12\n");
      display_info_add("","FAT12");
      break;

    case DOS_FAT16:
      printf("FAT16\n");
      display_info_add("","FAT16");
      break;

    case DOS_FAT32:
      printf("FAT32\n");
      display_info_add("","FAT32");
      break;

    default:
      printf("Unknown FAT type\n\n");
      display_info_add("","Unknown FAT");
      return;
  }
...
int dos_validate()
{
  ...
  if (sector1==NULL) 
	  {fprintf(stderr,"could't find 0,0,1\n");
    return DOS_UNKNOWN;}

  if (sector1->data==NULL)
  {fprintf(stderr,"could't find data for 0,0,1\n");
    return DOS_UNKNOWN;}

  // Check sector is 512 bytes in length
  if (sector1->datasize!=DOS_SECTORSIZE)
  {fprintf(stderr,"not 512 bytes\n");
    return DOS_UNKNOWN;}
